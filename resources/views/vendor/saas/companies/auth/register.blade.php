        ...
        <script type="text/x-template" id="seller-registration">
            <div class="form-container">
                ...
                <form class="registration" data-vv-scope="step-one" v-if="step_one" @submit.prevent="validateForm('step-one')">
                    ...
                    <h3 class="mb-30">{{ __('saas::app.registration.step-1') }}:</h3>

                    <h4>{{ __('saas::app.registration.auth-cred') }}</h4>

                    <div class="control-group" :class="[errors.has('step-one.email') ? 'has-error' : '']">
                        <label for="email" class="required">{{ __('saas::app.registration.email') }}</label>

                        <input type="text" v-validate="'required|email|max:191'" class="control" v-model="email" name="email" data-vv-as="&quot;{{ __('saas::app.registration.email') }}&quot;" placeholder="Auth Email">
                    ...
                    <div class="control-group" :class="[errors.has('step-one.password') ? 'has-error' : '']">
                        <label for="password" class="required">{{ __('saas::app.registration.password') }}</label>

                        <input type="password" name="password" v-validate="'required|min:6'" ref="password" class="control" v-model="password" placeholder="password" data-vv-as="&quot;{{ __('saas::app.registration.password') }}&quot;">
                    ...
                    <div class="control-group" :class="[errors.has('step-one.password_confirmation') ? 'has-error' : '']">
                        <label for="password_confirmation" class="required">{{ __('saas::app.registration.cpassword') }}</label>

                        <input type="password" v-validate="'required|min:6|confirmed:password'" class="control" v-model="password_confirmation" name="password_confirmation" placeholder="Confirm Password" data-vv-as="&quot;{{ __('saas::app.registration.cpassword') }}&quot;">
                    ...
                    <div class="control-group">
                        ...
                        <button class="btn btn-lg btn-primary" :disabled="errors.has('step-one.password') || errors.has('step-one.password_confirmation') || errors.has('step-one.email')">{{ __('saas::app.registration.continue') }}</button>
                    </div>
                </form>

                <form class="registration" @submit.prevent="validateForm('step-two')" data-vv-scope="step-two" v-show="step_two">
                    <div class="step-two">
                        <h3 class="mb-30">{{ __('saas::app.registration.step-2') }}:</h3>

                        <h4>{{ __('saas::app.registration.personal') }}</h4>

                        <div class="control-group" :class="[errors.has('step-two.first_name') ? 'has-error' : '']" >
                            <label for="first_name" class="required">{{ __('saas::app.registration.first-name') }}</label>

                            <input type="text" class="control" v-model="first_name" name="first_name" placeholder="First Name" v-validate="'required|alpha_spaces'" data-vv-as="&quot;{{ __('saas::app.registration.first-name') }}&quot;">
                        ...
                        <div class="control-group" :class="[errors.has('step-two.last_name') ? 'has-error' : '']">
                            <label for="last_name">{{ __('saas::app.registration.last-name') }}</label>

                            <input type="text" class="control" name="last_name" v-model="last_name" placeholder="{{ __('saas::app.registration.last-name') }}" v-validate="'alpha_spaces'" data-vv-as="&quot;{{ __('saas::app.registration.first-name') }}&quot;">
                        ...
                        <div class="control-group" :class="[errors.has('step-two.phone_no') ? 'has-error' : '']">
                            <label for="phone_no" class="required">{{ __('saas::app.registration.phone') }}</label>

                            <input type="text" class="control" name="phone_no" v-model="phone_no" placeholder="Phone Number" v-validate="'required'" data-vv-as="&quot;{{ __('saas::app.registration.phone') }}&quot;">

                            <span class="control-error" v-show="errors.has('step-two.phone_no')">@{{ errors.first('step-two.phone_no') }}</span>
                        </div>

                        <div class="control-group">
                            <button class="btn btn-lg btn-primary" :disabled="errors.has('first_name') || errors.has('last_name') || errors.has('step-two.phone_no')">{{ __('saas::app.registration.continue') }}</button>
                        </div>
                    </div>
                </form>
                ...
                <form class="registration" @submit.prevent="validateForm('step-three')" data-vv-scope="step-three" v-show="step_three">
                    <div class="step-three">
                        <h3 class="mb-30">{{ __('saas::app.registration.step-3') }}:</h3>

                        <h4>{{ __('saas::app.registration.org-details') }}:</h4>

                        <div class="control-group" :class="[errors.has('step-three.username') ? 'has-error' : '']">
                            <label for="username" class="required">User Name</label>

                            <input type="text" class="control" name="username" v-model="username" placeholder="{{ __('saas::app.registration.username') }}" v-validate="'required|alpha_num'" data-vv-as="&quot;{{ __('saas::app.registration.username') }}&quot;">
                        ...
                        <div class="control-group" :class="[errors.has('step-three.org_name') ? 'has-error' : '']">
                            <label for="org_name" class="required">{{ __('saas::app.registration.org-name') }}</label>

                            <input type="text" class="control" name="org_name" v-model="org_name" placeholder="{{ __('saas::app.registration.org-name') }}" v-validate="'required'" data-vv-as="&quot;{{ __('saas::app.registration.org-name') }}&quot;">

                            <span class="control-error" v-show="errors.has('step-three.org_name')">@{{ errors.first('step-three.org_name') }}</span>
                        </div>

                        <div class="control-group">
                            <button class="btn btn-lg btn-primary" :disabled="errors.has('step-three.username') || errors.has('step-three.org_name') || createdclicked">{{ __('saas::app.registration.continue') }}</button>
                        </div>
                    </div>
                </form>
            </div>
